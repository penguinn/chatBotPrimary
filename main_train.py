...
def predict():
    ...
    with tf.Session() as sess:
        ...
        while True:
            handle.eventEngine.wait()
            handle.eventEngine.clear()
            input_seq = handle.question.strip()
            ...
            if (len(input_id_list)):
                ...
                handle.answer =  " ".join(outputs_seq).encode("utf-8")
            else:
                handle.answer =  "不想理你"
            handle.eventHandle.set()

def predict_test(question):
    """
    预测过程
    """
    with tf.Session() as sess:
        ...
        if (len(input_id_list)):
            ...
            outputs_seq = [wordToken.id2word(v) for v in outputs_seq]
            handle.answer =  " ".join(outputs_seq).encode("utf-8")
            print chardet.detect(handle.answer)
        else:
            handle.answer =  "test"
            print chardet.detect(handle.answer)
